...
<input type="hidden" value="${path }" id="path"/>
	<div class="col-md-12">
		<div class="x_panel">
			...
			<div class="x_content">
				<form method="post" action="${pageContext.request.contextPath }/appInfo/applist.html" modelAttribute="appInfo"> 
								...
								<select name="status" class="form-control">
									<c:if test="${status != null }">
									   ...
									   <c:forEach var="dataDictionary" items="${status}">
									   		<option <c:if test="${dataDictionary.valueid == info.status }">selected="selected"</c:if>
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select name="flatformid" class="form-control">
									<c:if test="${from != null }">
									   <option value="">--请选择--</option>
									   <c:forEach var="dataDictionary" items="${from}">
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select id="queryCategoryLevel1" name="categorylevel1" class="form-control">
									<c:if test="${one != null }">
									   <option value="">--请选择--</option>
									   <c:forEach var="appCategory" items="${one}">
        						...
        						<select name="categorylevel2" id="queryCategoryLevel2" class="form-control">
        							<c:if test="${categoryLevel2List != null }">
									   ...
									   <c:forEach var="appCategory" items="${categoryLevel2List}">
									   		<option <c:if test="${appCategory.id == info.categorylevel2 }">selected="selected"</c:if>
									   		value="${appCategory.id}">${appCategory.categoryname}</option>
									   </c:forEach>
									</c:if>
        						</select>
        						...
        						<select name="categorylevel3" id="queryCategoryLevel3" class="form-control">
        							<c:if test="${categoryLevel3List != null }">
									   ...
									   <c:forEach var="appCategory" items="${categoryLevel3List}">
									   		<option <c:if test="${appCategory.id == info.categorylevel3 }">selected="selected"</c:if>
									   		value="${appCategory.id}">${appCategory.categoryName}</option>
									   </c:forEach>
									</c:if>
        						</select>
	...
	<div class="x_panel">
		<div class="x_content">
			...
			<div id="datatable-responsive_wrapper"
				...
				<div class="row">
					<div class="col-sm-12">
					<a href="${pageContext.request.contextPath}/appInfo/appadd.html" class="btn btn-success btn-sm">新增APP基础信息</a>
							...
							<ul class="pagination">
								<c:if test="${pageinfo.pageNum > 1}">
									<li class="paginate_button previous"><a
										href="${pageContext.request.contextPath }/appInfo/applist.html?pn=1&flag=select"
										aria-controls="datatable-responsive" data-dt-idx="0"
										tabindex="0">首页</a>
									</li>
									<li class="paginate_button "><a
										href="${pageContext.request.contextPath }/appInfo/applist.html?pn=${pageinfo.pageNum-1}&flag=select"
								...
								<c:if test="${pageinfo.pageNum < pageinfo.pages }">
									...
									<li class="paginate_button next"><a
										href="${pageContext.request.contextPath }/appInfo/applist.html?pn=${pageinfo.pages }&flag=select"
