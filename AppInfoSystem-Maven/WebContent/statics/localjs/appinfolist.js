...
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href=path+"/appVersion/appversionupdate.html?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href=path+"/appInfo/update.html?id="+ obj.attr("appinfoid");
	}else{
...
var saleSwitchAjax = function(appId,obj){
	...
	$.ajax({
		type:"post",
		url:path+"/appInfo/updatestatus?id="+appId+"&sale="+sale,
		dataType:"json",
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	var path=$("#path").val();
	window.location.href=path+"/appInfo/appinfoview.html?id="+ obj.attr("appinfoid");
});
